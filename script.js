...
// 🔥 Speichern in Local Storage & GitHub
function saveData() {
    localStorage.setItem("members", JSON.stringify(members));
    saveToGitHub();
}

// 📤 **Speichern in GitHub Repository**
function saveToGitHub() {
    const githubToken = "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"; // Ersetze durch dein GitHub Token
    const owner = "erhanm61"; // Ersetze durch den GitHub-Nutzer oder die Organisation
    const repo = "LosTurcos"; // Ersetze durch den Repository-Namen
    const path = "members.json"; // Datei im Repository
    const apiUrl = `https://api.github.com/repos/${owner}/${repo}/contents/${path}`;

    const fileContent = JSON.stringify(members, null, 2);
    const encodedContent = btoa(unescape(encodeURIComponent(fileContent))); // Base64-Encodierung für GitHub API

    // Zuerst den aktuellen SHA-Wert abrufen
    fetch(apiUrl, {
        method: "GET",
        headers: {
    ...
    .then(data => {
        const sha = data.sha; // SHA des bestehenden Files
        ...
        fetch(apiUrl, {
            ...
            body: JSON.stringify({
                message: "Mitgliederliste aktualisiert",
                content: encodedContent,
                sha: sha // Notwendig für Updates
            })
        })
        .then(response => response.json())
        .then(data => console.log("✅ Erfolgreich in GitHub gespeichert:", data))
        .catch(error => console.error("❌ Fehler beim Speichern in GitHub:", error));
    })
    .catch(error => {
        console.error("❌ Fehler beim Abrufen der Datei SHA:", error);
    });
}
